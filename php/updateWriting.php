...
$query = "UPDATE `sns_post` SET `contents`=?, `isprivate_num`=?, `link`=?, `comment_private_num`=? where `userid`=? and `id`=?";
$bRes = execsql($con, $query, [$contents, $isprivatenum, $link, $comment_private_num, $userid, $postid]);
